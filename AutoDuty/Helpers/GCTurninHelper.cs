...
namespace AutoDuty.Helpers
{
    internal class GCTurninHelper : ActiveHelperBase<GCTurninHelper>
    {
        ...
        protected override unsafe void HelperUpdate(IFramework framework)
        {
            ...
            if (!this._deliverooStarted && Deliveroo_IPCSubscriber.IsTurnInRunning())
            {
                InfoLog("TurnIn has Started");
                this._deliverooStarted = true;
                return;
            }
            else if (this._deliverooStarted && !Deliveroo_IPCSubscriber.IsTurnInRunning())
            {
            ...
            else if (ObjectHelper.GetDistanceToPlayer(GCSupplyLocation) <= 4 && VNavmesh_IPCSubscriber.Path_NumWaypoints() == 0 && !this._deliverooStarted)
            {
